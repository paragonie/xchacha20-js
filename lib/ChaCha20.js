...
module.exports = class ChaCha20
{
     ...
     * @return {Buffer}
     */
    ietfStream(len = 64, nonce = null, key = null)
    {
        if (!key) {
     ...
     * @return {Buffer}
     */
    ietfStreamIc(len = 64, nonce = null, key = null, counter = 0)
    {
        if (!key) {
            key = Buffer.alloc(32, 0);
        }
     ...
     * @return {Buffer}
     */
    stream(len = 64, nonce = null, key = null)
    {
        if (!key) {
            key = Buffer.alloc(32, 0);
        }
     ...
     * @return {Buffer}
     */
    streamIc(len = 64, nonce = null, key = null, counter = 0)
    {
        if (!key) {
            key = Buffer.alloc(32, 0);
        }
     ...
     */
    ietfStreamXorIc(message, nonce, key, ic = 0)
    {
        if (nonce.length !== 12) {
            throw new Error('Nonce must be 12 bytes')
        }
     ...
     * @param {Buffer|Number} ic
     */
    streamXorIc(message, nonce, key, ic = 0)
    {
        if (nonce.length !== 8) {
            throw new Error('Nonce must be 8 bytes')
        }
